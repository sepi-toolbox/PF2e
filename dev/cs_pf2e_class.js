...
(function (root) {
  ...
  function loadFamiliarData() {
    ...
    return Promise.all([
      fetch('data/derived/familiar_progression.json?v=0.234').then(r => r.json()).then(_fillFamProg).catch(() => {}),
      fetch('data/derived/familiar_abilities.json?v=0.234').then(r => r.json()).then(j => _fillFamAbil(j.rows)).catch(() => {}),
    ]).then(() => {});
  }
  ...
  async function _ensureSubProfTable() {
    ...
    if (rows == null) { try { const r = await fetch("data/derived/subclass_progression.json?v=0.234"); rows = ((await r.json()).rows) || []; } catch (e) { rows = []; } }
  ...
  async function _ensureProfTable() {
    ...
    let rows = _profRows();
    if (rows == null) { try { const r = await fetch("data/derived/class_progression.json?v=0.234"); rows = ((await r.json()).rows) || []; } catch(e){ rows = []; } }
  ...
  function loadDoctrines() {
    ...
    return fetch('data/derived/cleric_doctrines.json?v=0.234').then(r => r.json()).then(j => { _injectDoctrines(j.rows); _doctrinesLoaded = true; }).catch(() => {});
  }
  ...
  function loadSubclasses() {
    ...
    return fetch('data/derived/subclasses.json?v=0.234').then(r => r.json()).then(j => { inject(j.rows); _subclassesLoaded = true; }).catch(() => {});
  }
  ...
  function loadBloodlines() {
    ...
    return fetch('data/derived/bloodlines.json?v=0.234').then(r => r.json()).then(j => { _fillBloodlineDB(j.rows); _fillBloodlineGuide(j.guide); _bloodlinesLoaded = true; }).catch(() => {});
  }
  ...
  function loadMysteries() {
    ...
    return fetch('data/derived/oracle_mysteries.json?v=0.234').then(r => r.json()).then(j => { _fillMysteryDB(j.rows); _fillMysteryGuide(j.guide); _mysteriesLoaded = true; }).catch(() => {});
  }
  ...
  function loadWizardSchools() {
    ...
    return fetch('data/derived/wizard_schools.json?v=0.234').then(r => r.json()).then(j => { _fillWizardSchoolDB(j.rows); _fillWizardSchoolGuide(j.guide); _wizardSchoolsLoaded = true; }).catch(() => {});
  }
  ...
  function loadBardMuses() {
    ...
    return fetch('data/derived/bard_muses.json?v=0.234').then(r => r.json()).then(j => { _fillBardMuseGuide(j.guide); _bardMusesLoaded = true; }).catch(() => {});
  }
  ...
  function loadWitchPatrons() {
    ...
    return fetch('data/derived/witch_patrons.json?v=0.234').then(r => r.json()).then(j => { _fillWitchPatronGuide(j.guide); _witchPatronsLoaded = true; }).catch(() => {});
  }
  ...
  function loadDruidOrders() {
    ...
    return fetch('data/derived/druid_orders.json?v=0.234').then(r => r.json()).then(j => { _fillDruidOrderGuide(j.guide); _druidOrdersLoaded = true; }).catch(() => {});
  }
  ...
  function loadRangerEdges() {
    ...
    return fetch('data/derived/ranger_edges.json?v=0.234').then(r => r.json()).then(j => { _fillRangerEdgeGuide(j.guide); _rangerEdgesLoaded = true; }).catch(() => {});
  }

  // 로그 라켓 가이드 = data/derived/rogue_rackets.json → ROGUE_RACKET_GUIDE(모달 「라켓 항목 읽는 법」).
  ...
  function loadRogueRackets() {
    ...
    return fetch('data/derived/rogue_rackets.json?v=0.234').then(r => r.json()).then(j => { _fillRogueRacketGuide(j.guide); _rogueRacketsLoaded = true; }).catch(() => {});
  }
  ...
  async function init() {
    ...
    if (isNode) loadRogueRackets(); else await loadRogueRackets();    // 로그 라켓 가이드
