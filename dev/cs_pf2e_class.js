...
(function (root) {
  ...
  async function _ensureSubProfTable() {
    ...
    if (rows == null) { try { const r = await fetch("data/derived/subclass_progression.json?v=0.202"); rows = ((await r.json()).rows) || []; } catch (e) { rows = []; } }
  ...
  async function _ensureProfTable() {
    ...
    let rows = _profRows();
    if (rows == null) { try { const r = await fetch("data/derived/class_progression.json?v=0.202"); rows = ((await r.json()).rows) || []; } catch(e){ rows = []; } }
  ...
  function loadDoctrines() {
    ...
    return fetch('data/derived/cleric_doctrines.json?v=0.202').then(r => r.json()).then(j => { _injectDoctrines(j.rows); _doctrinesLoaded = true; }).catch(() => {});
  }
  ...
  function loadSubclasses() {
    ...
    return fetch('data/derived/subclasses.json?v=0.202').then(r => r.json()).then(j => { inject(j.rows); _subclassesLoaded = true; }).catch(() => {});
  }
  ...
  function loadBloodlines() {
    ...
    return fetch('data/derived/bloodlines.json?v=0.202').then(r => r.json()).then(j => { _fillBloodlineDB(j.rows); _fillBloodlineGuide(j.guide); _bloodlinesLoaded = true; }).catch(() => {});
  }
