...
(function (root) {
  ...
  async function _ensureSubProfTable() {
    ...
    if (rows == null) { try { const r = await fetch("data/derived/subclass_progression.json?v=0.220"); rows = ((await r.json()).rows) || []; } catch (e) { rows = []; } }
  ...
  async function _ensureProfTable() {
    ...
    let rows = _profRows();
    if (rows == null) { try { const r = await fetch("data/derived/class_progression.json?v=0.220"); rows = ((await r.json()).rows) || []; } catch(e){ rows = []; } }
  ...
  function loadDoctrines() {
    ...
    return fetch('data/derived/cleric_doctrines.json?v=0.220').then(r => r.json()).then(j => { _injectDoctrines(j.rows); _doctrinesLoaded = true; }).catch(() => {});
  }
  ...
  function loadSubclasses() {
    ...
    return fetch('data/derived/subclasses.json?v=0.220').then(r => r.json()).then(j => { inject(j.rows); _subclassesLoaded = true; }).catch(() => {});
  }
  ...
  function loadBloodlines() {
    ...
    return fetch('data/derived/bloodlines.json?v=0.220').then(r => r.json()).then(j => { _fillBloodlineDB(j.rows); _fillBloodlineGuide(j.guide); _bloodlinesLoaded = true; }).catch(() => {});
  }
  ...
  function loadMysteries() {
    ...
    return fetch('data/derived/oracle_mysteries.json?v=0.220').then(r => r.json()).then(j => { _fillMysteryDB(j.rows); _fillMysteryGuide(j.guide); _mysteriesLoaded = true; }).catch(() => {});
  }
  ...
  function loadWizardSchools() {
    ...
    return fetch('data/derived/wizard_schools.json?v=0.220').then(r => r.json()).then(j => { _fillWizardSchoolDB(j.rows); _fillWizardSchoolGuide(j.guide); _wizardSchoolsLoaded = true; }).catch(() => {});
  }
  ...
  function loadBardMuses() {
    ...
    return fetch('data/derived/bard_muses.json?v=0.220').then(r => r.json()).then(j => { _fillBardMuseGuide(j.guide); _bardMusesLoaded = true; }).catch(() => {});
  }
...
const API = { init, ready, rebuild, classList, getClassLegacy, classToLegacy, classProfTable, subclassProfTable, subclassGrantTable, isLegacy, classFeatures, classFeatureRoster, subclassList, spellTable, loadBloodlines, loadMysteries, loadWizardSchools, loadBardMuses };
  root.PF2eClass = API;
