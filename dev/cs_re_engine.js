...
(function (root) {
      ...
      for (const r of rules) {
        switch (r.key) {
          ...
          case 'GrantItem': {
            ...
            if (g) { const ge = { doc: g, choices: {}, _granted: true, _grantedBy: slug }; a.items.push(ge); queue.push(ge); a.grantedItems.push(g.name_ko || g.name); }
            else a._log.push(`GrantItem 미해소: ${r.uuid} (from ${doc.name})`);
            break;
          }
          case 'ActiveEffectLike': _applyAELike(a, r, ctx); break;
          case 'Sense': if (r.selector) a.senses.add(r.selector); break;
          case 'BaseSpeed': if (r.selector) { const v = Number(PF.resolveBrackets(r.value, ctx)) || 0; a.speeds[r.selector] = Math.max(a.speeds[r.selector] || 0, v); } break;
  ...
  function _applyAELike(a, r, ctx) {
    ...
    let val = PF.resolveBrackets(r.value, ctx); const num = Number(val);
    // 능력치/속도 등 알려진 수치 경로만 v1 처리
    let target = null, key = null;
    const mAb = path.match(/^system\.abilities\.(\w+)\.mod$/);
    if (mAb && ABILITIES.includes(mAb[1])) { target = a.abilities; key = mAb[1]; }
    if (target && !Number.isNaN(num)) {
      const cur = target[key] || 0;
      if (mode === 'add') target[key] = cur + num;
      else if (mode === 'subtract') target[key] = cur - num;
      else if (mode === 'upgrade') target[key] = Math.max(cur, num);
      else if (mode === 'downgrade') target[key] = Math.min(cur, num);
      else if (mode === 'override') target[key] = num;
      else if (mode === 'multiply') target[key] = cur * num;
      a.aeApplied.push(`${path} ${mode} ${num} → ${target[key]}`);
    } else {
      a.aeApplied.push(`(skip) ${path} ${mode} ${val}`);
    }
  }
      ...
      for (const r of rules) {
        ...
        switch (r.key) {
          case 'FlatModifier': {
            ...
            const val = Number(PF.resolveBrackets(r.value, ctx));
          ...
          case 'Resistance': a.iwr.resistances.push({ type: PF.resolveBrackets(r.type, ctx), value: Number(PF.resolveBrackets(r.value, ctx)) || r.value, source: src }); break;
          case 'Weakness': a.iwr.weaknesses.push({ type: PF.resolveBrackets(r.type, ctx), value: Number(PF.resolveBrackets(r.value, ctx)) || r.value, source: src }); break;
          ...
          case 'AdjustModifier': {
            ...
            const delta = Number(PF.resolveBrackets(r.value, ctx)); if (Number.isNaN(delta)) break;
