...
function _loadIconMap() {
  if (_ICON_MAP) return;
  fetch('data/icon_map.json?v=0.254').then(r => r.ok ? r.json() : null).then(m => {
    if (!m) return;
    _ICON_MAP = m;
...
function _loadCompanions() {
  ...
  fetch('data/derived/companions.json?v=0.254').then(r => r.ok ? r.json() : null).then(j => {
...
function _loadConditions() {
  ...
  fetch('data/derived/conditions.json?v=0.254').then(r => r.ok ? r.json() : null).then(j => {
...
function _familiarAbilityIconUrl(id) { return FAMILIAR_ABILITY_ICON_BASE + (FAMILIAR_ABILITY_ICONS[id] || FAMILIAR_DEFAULT_ICON) + '?v=0.254'; }
function _familiarPatronIconUrl() { return FAMILIAR_ABILITY_ICON_BASE + FAMILIAR_PATRON_ICON + '?v=0.254'; }
...
function _familiarAbilityBoxHtml(icon, name, sub, desc, locked) {
  return `<div style="display:flex;align-items:flex-start;gap:8px;padding:6px 8px;background:var(--bg3);border:1px solid ${locked ? 'var(--accent)' : 'var(--border2)'};border-radius:6px;">
    <img src="${icon}" loading="lazy" style="width:28px;height:28px;border-radius:5px;flex-shrink:0;object-fit:cover;" onerror="this.src='${FAMILIAR_ABILITY_ICON_BASE + FAMILIAR_DEFAULT_ICON}?v=0.254'">
