...
function _loadIconMap() {
  if (_ICON_MAP) return;
  fetch('data/icon_map.json?v=0.321').then(r => r.ok ? r.json() : null).then(m => {
    if (!m) return;
    _ICON_MAP = m;
...
function renderArmorCard() {
  ...
  // 착용 갑옷 아이콘 = 착용 equip 인스턴스의 _data.img (무기 카드와 동일 방식). equip 없으면 이름으로 icon_map 폴백.
  const _aEq = worn ? (_wornArmorEquipIdx() >= 0 ? state.equip[_wornArmorEquipIdx()] : { name_ko: name }) : null;
  const _aIcon = worn ? (iconImg('equipment', _aEq) || '') : '';
  ...
  card.innerHTML = `
    ...
    <div class="weapon-card-body">
      <div class="weapon-card-name" ${worn ? `onclick="showInfo('armor','${name.replace(/'/g, "\\'")}')"` : ''}>
        ${_aIcon}<span class="prof-rank-badge ${profCls}">${profLetter}</span> ${displayName}
        ...
        ${state.armorMaterial ? `<span class="wc-flag" style="color:var(--gold,#c9a44a);">${(typeof _matKo==='function'&&_matKo(state.armorMaterial))||''}</span>` : ''}
        ${state.armorBulkOverride != null ? `<span class="wc-flag" style="color:var(--text2);">부피 ${state.armorBulkOverride===0.1?'L':state.armorBulkOverride}</span>` : ''}
      </div>
...
function renderShieldCard() {
  ...
  card.innerHTML = `
    ...
    <div class="weapon-card-body">
      <div class="weapon-card-name" onclick="showInfo('shield','${name.replace(/'/g, "\\'")}')">${iconImg('equipment', _sEq || { name_ko: name }) || ''}${name}</div>
      <div class="weapon-stat-row">
...
// ── 방어 카드 액션(변경/옵션/룬/제거) ──
// "변경" = 방어구 전용 브라우저(무기 브라우저 패턴) + 지급 시 자동 착용(_equipGiveMode).
let _equipGiveMode = null;  // 'wear-armor' | 'equip-shield' | null
let armorBrowseCat = '';               // '' | light | medium | heavy | unarmored
let armorBrowseKind = 'standard';      // standard(비마법) | magic(마법 트레잇)
function openArmorBrowse() {
  equipBrowseMode = 'armor';
  armorBrowseCat = '';
  armorBrowseKind = 'standard';
  ...
  document.getElementById('modal-title').textContent = '갑옷 변경';
  ...
  if (!tabContainer) {
    ...
    // ⚠ document.querySelector('.modal') 금지(다중 .modal) — #modal-body의 실제 부모(메인 모달)에 삽입.
    const modalBody = document.getElementById('modal-body');
    modalBody.parentNode.insertBefore(tabContainer, modalBody);
  }
  tabContainer.style.display = '';
  const cats = [['', '전체'], ['light', '경장'], ['medium', '중장'], ['heavy', '중량'], ['unarmored', '비무장']];
  tabContainer.innerHTML = `
    <div class="equip-tabs" id="armor-cat-tabs">
      ${cats.map(([id, ko]) => `<div class="equip-tab${id === '' ? ' active' : ''}" onclick="switchArmorCat('${id}')">${ko}</div>`).join('')}
    </div>
    <div class="equip-subtabs" id="armor-kind-tabs">
      <div class="equip-subtab active" onclick="switchArmorKind('standard')">표준</div>
      <div class="equip-subtab" onclick="switchArmorKind('magic')">마법</div>
    </div>`;
  ...
  renderArmorBrowseItems();
}

function switchArmorCat(cat) {
  armorBrowseCat = cat;
  const order = ['', 'light', 'medium', 'heavy', 'unarmored'];
  const tabs = document.querySelectorAll('#armor-cat-tabs .equip-tab');
...
function switchArmorKind(kind) {
  armorBrowseKind = kind;
  document.querySelectorAll('#armor-kind-tabs .equip-subtab').forEach(t => {
    t.classList.toggle('active', (t.textContent === '표준' && kind === 'standard') || (t.textContent === '마법' && kind === 'magic'));
  });
  renderArmorBrowseItems();
}

function renderArmorBrowseItems() {
  const opts = document.getElementById('modal-options');
  if (!_equipDataReady) {
    if (opts) opts.innerHTML = '<div style="color:var(--text2);text-align:center;padding:20px;">갑옷 데이터 불러오는 중…</div>';
    _ensureEquipData().then(ok => {
      if (ok) renderArmorBrowseItems();
      else if (opts) opts.innerHTML = '<div style="color:var(--text2);text-align:center;padding:20px;">갑옷 데이터를 불러오지 못했습니다.</div>';
    });
    return;
  }
  const q = (document.getElementById('modal-search')?.value || '').toLowerCase();
  // 동물 바딩(light-barding/heavy-barding)은 PC 갑옷 아님 → 제외.
  const all = PF2eEquip.legacyList({ type: 'armor', search: q }).filter(i => (i.catSlug || '') !== 'light-barding' && (i.catSlug || '') !== 'heavy-barding');
  // 표준 / 마법 = 'magical' 트레잇 기준(구체 마법갑옷·특정 매직아이템만 마법 탭으로 분리).
  const magicIds = new Set(PF2eEquip.legacyList({ type: 'armor', search: q, traits: ['magical'] }).map(i => i.id));
  let items = all.filter(i => armorBrowseKind === 'magic' ? magicIds.has(i.id) : !magicIds.has(i.id));
  if (armorBrowseCat) items = items.filter(i => (i.catSlug || '') === armorBrowseCat);
  if (opts && !items.length) { opts.innerHTML = '<div style="color:var(--text2);text-align:center;padding:20px;">해당 조건의 갑옷이 없습니다.</div>'; return; }
  renderOptions(items);
}
...
function _ensureArmorMatData() {
  ...
  _armorMatPromise = fetch('data/armor_materials.json?v=0.321')
...
function _armorOptAcParts() {
  ...
  const item = worn ? baseAc + (state.armorPotency || 0) : 0;
  const dexCapV = document.getElementById('armor-dex')?.value;
  let dex = (typeof getMod === 'function') ? getMod('dex') : 0;
  if (worn && dexCapV && dexCapV !== '-' && dexCapV !== '') dex = Math.min(dex, parseInt(dexCapV) || 99);
  const profId = (typeof getArmorProfSelectId === 'function') ? getArmorProfSelectId() : 'prof-armor-unarmored';
...
function openArmorOptions(ev) {
  ...
  _aoptSnap = {
    potency: state.armorPotency || 0,
    resilient: state.armorResilient || 0,
...
function closeArmorOptions(cancel) {
  if (cancel && _aoptSnap) {
    state.armorPotency = _aoptSnap.potency;
    state.armorResilient = _aoptSnap.resilient;
    state.armorMaterial = _aoptSnap.material;
    state.armorBulkOverride = _aoptSnap.bulk;
...
function _renderArmorOpt() {
  ...
  const field = (label, val, view) => `
    <div class="aopt-row" onclick="_aoptGo('${view}')">
      <span class="aopt-label">${label}</span>
      <button class="aopt-pick">${val}</button>
    </div>`;
...
function _temlScaleHtml(rank) {
  ...
  return ranks.map(([c, rv]) => `<span class="teml-c${rank === rv ? ' on' : ''}">${c}</span>`).join('');
}
// 무기 탭 상단 4범주 숙련 스케일 갱신 (syncAllProfRanks에서 호출)
function updateWeaponTeml() {
...
function switchSpellSubtab(tab) {
  // '신성 원천(font)'은 별도 콘텐츠 없이 클래스 콘텐츠를 공유(view-font 클래스로 캔트립/랭크 숨김, 신성 원천만)
  const classContent = document.getElementById('spell-content-class');
  if (classContent) classContent.classList.toggle('view-font', tab === 'font');
  ['class','font','focus','innate','ritual'].forEach(t => {
    const tabEl = document.getElementById('spell-subtab-'+t);
    if (tabEl) tabEl.classList.toggle('active', tab===t);
    const contentEl = document.getElementById('spell-content-'+t);   // font는 content div 없음
    if (contentEl) contentEl.style.display = (t===tab || (t==='class' && tab==='font')) ? '' : 'none';
  });
}
...
function updateSpellTemlBadges() {
  ...
  // 헤더 TEML 배지(현재 주문시전 숙련 등급 강조, 클릭=등급 설정)
  const el = document.getElementById('prof-spatk');
  const rank = el ? parseInt(el.value || 0) : 0;
  const map = { 2: 't', 4: 'e', 6: 'm', 8: 'l' };
  const html = [['t', 2], ['e', 4], ['m', 6], ['l', 8]].map(([c, rv]) =>
    `<span class="teml ${c} ${map[rank] === c ? 'on' : ''}" onclick="setSpellProf(${rv})" style="cursor:pointer;">${c.toUpperCase()}</span>`
  ).join('');
  ['spell-teml', 'spell-teml-focus'].forEach(id => { const t = document.getElementById(id); if (t) t.innerHTML = html; });
}

// ── 보너스 주문 모달(예시 Pathbuilder): 랭크별 추가 주문 슬롯 ──
function openBonusSpellsModal() {
  if (!state.bonusSpellSlots) state.bonusSpellSlots = {};
  const maxRank = Math.min(10, Math.ceil((getLevel() || 1) / 2)) || 1;
  const clsName = state.selectedClass ? state.selectedClass.name : '';
  document.getElementById('modal-overlay').classList.remove('hidden');
  document.getElementById('modal-title').textContent = `${clsName} 보너스 주문`.trim();
  ...
  modalType = 'bonus-spells';
  let rows = '';
  for (let r = 0; r <= maxRank; r++) {
    const cur = state.bonusSpellSlots[r] || 0;
    const label = r === 0 ? '캔트립 (랭크 0)' : `${r}랭크 주문`;
    const opts = [0, 1, 2, 3, 4, 5].map(n => `<option value="${n}" ${n === cur ? 'selected' : ''}>+${n}</option>`).join('');
    rows += `<div class="bonus-spell-row"><div class="bonus-spell-label">${label}</div><select class="bonus-spell-sel" onchange="_onBonusSpellChange(${r},this.value)">${opts}</select></div>`;
  }
...
function _onBonusSpellChange(r, val) {
  ...
  if (n) state.bonusSpellSlots[r] = n; else delete state.bonusSpellSlots[r];
...
function renderSpells() {
  ...
  const allSubtabs = ['class','font','focus','innate','ritual'];
  ...
  // v0.299: "배우기" 폐지 — 모든 시전자를 "전통 전체 준비(클레릭식)"로 통합.
  //   즉흥(바드/소서러)도 슬롯에 전통 전체 풀에서 준비. 레퍼토리·시그니처·주문서 습득한도 개념 폐지.
  //   접근 불가 주문 필터(전통 게이트)는 memorize 모달에서 유지.
  const hasSpellTable = (typeof CLASS_SPELL_TABLE !== 'undefined') && !!CLASS_SPELL_TABLE[state.selectedClass?.id];
  const hasCasting = !!(state.selectedClass?.casting);
  const isPreparedCaster = hasCasting && hasSpellTable;   // 전 시전자 = 준비형 렌더
  const isPrepared = hasCasting;                          // 메모라이즈 버튼 게이트(전 시전자)
  const isSpontaneous = false;                            // 즉흥 전용 경로 폐지
  const memBtn = document.getElementById('btn-memorize-spells');
  if (memBtn) memBtn.style.display = isPrepared ? '' : 'none';
  // "주문 배우기" 단계 폐지(v0.298) — 주문서형(위저드/위치)도 전통 전체에서 준비(클레릭식) → 배우기 버튼 숨김.
  //   spontaneous(바드/소서러)는 아직 레퍼토리 방식이라 유지(후속 단계에서 전통 전체로 통합 예정).
  const showLearnBtn = isSpontaneous;
  ...
  const cantripSlots = (state.cantripSlots || 5) + ((state.bonusSpellSlots && state.bonusSpellSlots[0]) || 0);
  ...
  const fontTab = document.getElementById('spell-subtab-font');
  const hasFont = !!(state.divineFont && state.selectedClass?.deity_skill && !(state._invalidChoices && state._invalidChoices.divineFont));
  if (fontTab) fontTab.style.display = hasFont ? '' : 'none';   // 신성 원천 서브탭은 원천 있을 때만
  if (dfSection && dfBody) {
    // 신격과 불일치(선행조건 불일치)면 신성 원천 효과 보류 → 주문 탭 섹션 숨김(B안)
    if (hasFont) {
      ...
      const dfRange = (dfSpell && dfSpell.range) ? dfSpell.range : '가변';
      // 예시(Pathbuilder)식: 슬롯마다 「시전 | 주문명 | 사거리」 행 N개(1행+pip → 슬롯별 Cast 행).
      let rows = '';
      for (let i = 0; i < totalSlots; i++) {
        const isUsed = i >= (totalSlots - used);
        rows += `<div class="spell-slot-row${isUsed ? ' slot-used' : ''}">
          <span class="spell-cast-label${isUsed ? ' cast-used' : ''}" onclick="toggleDivineFontSlot(${i})" title="${isUsed ? '슬롯 복원' : '시전(슬롯 사용)'}">시전</span>
          ...
          <span class="spell-slot-range">${dfRange}</span>
        </div>`;
      }
      dfBody.innerHTML = `<div class="spell-slot-col-header"><span class="spell-cast-label" style="visibility:hidden;">시전</span><span style="flex:1;">주문</span><span style="width:70px;text-align:center;">사거리</span></div>${rows}`;
    } else {
  ...
  for (let r = 1; r <= maxRank; r++) {
    const slotMax = parseInt(state.spellSlots?.[r] || 0) + ((state.bonusSpellSlots && state.bonusSpellSlots[r]) || 0);
    ...
    if (slotMax > 0 && !isPreparedCaster) {
      ...
      firesDiv.style.justifyContent = 'center';   // 예시식: 랭크 헤더 아래 중앙 슬롯 pip
      firesDiv.innerHTML = '';
    ...
    if (isSpontaneous) {
      ...
      const colHeaderP = document.createElement('div');
      colHeaderP.className = 'spell-slot-col-header';
      colHeaderP.innerHTML = `<span style="width:40px;text-align:center;">시전</span><span style="flex:1;">주문</span><span style="width:70px;text-align:center;">지속</span><span style="width:70px;text-align:center;">사거리</span><span style="width:20px;"></span>`;
      section.appendChild(colHeaderP);
      for (let i = 0; i < slotMax; i++) {
        ...
        if (name) {
          const spellData = getSpell(name);
          const actions = getActionIcons(spellData?.actions);
          const dur = (spellData && spellData.duration) ? spellData.duration : '—';
          const rng = (spellData && spellData.range) ? spellData.range : '—';
          row.innerHTML = `
            <span class="spell-cast-label${isCast?' cast-used':''}" onclick="togglePreparedCast(${r},${i})" title="${isCast?'슬롯 복원':'시전 (소모)'}">시전</span>
            <span class="spell-slot-name" onclick="toggleSpellInline(this,'${String(name).replace(/'/g,"\\'")}')">${iconImg('spell', spellData||{name})}${(typeof spellDisplay==='function')?spellDisplay(name):name}${actions ? ' <span class="spell-actions-inline">'+actions+'</span>' : ''}</span>
            <span class="spell-slot-dur">${dur}</span>
            <span class="spell-slot-range">${rng}</span>
            <span class="spell-slot-del" onclick="unprepareSlot(${r},${i})" title="준비 해제">✕</span>`;
        } else {
          row.innerHTML = `
            <span class="spell-cast-label" style="opacity:0.35;">시전</span>
...
function renderSpellSlotList(elId, arr, type, heightenedLevel) {
  ...
  arr.forEach((s, i) => {
    ...
    const spellData = getSpell(s.name);
    const actions = getActionIcons(spellData?.actions);
    // 우측 랭크 라벨(참조 양식: "Cantrip 1" / "Focus 1") — 캔트립/집중 + 유효 랭크.
    let rightLabel = '';
    if (type === 'focus') {
      const isCantrip = spellData ? spellData.is_cantrip : false;
      const baseRank = spellData?.rank || 1;
      const effectiveRank = isCantrip ? (heightenedLevel || 1) : Math.max(baseRank, heightenedLevel || baseRank);
      rightLabel = `<span style="margin-left:auto;font-size:12px;color:var(--accent);font-weight:600;white-space:nowrap;">${isCantrip ? '캔트립' : '집중'} ${effectiveRank}</span>`;
    }
    const row = document.createElement('div');
    row.className = 'spell-slot-row';
    if (isAuto) row.style.cssText = 'border-left:3px solid var(--accent);background:rgba(100,160,255,0.06);';
    row.innerHTML = `
      <span class="spell-slot-name" onclick="toggleSpellInline(this,'${s.name.replace(/'/g,"\\'")}')">${iconImg('spell', spellData || s)}${s.name}${actions ? ' <span class="spell-actions-inline">'+actions+'</span>' : ''}</span>
      ${rightLabel}
      ${isAuto ? '' : `<span class="spell-slot-del" onclick="removeSpell('${type}',${i})" style="margin-left:10px;">✕</span>`}`;
    el.appendChild(row);
  });
}
...
function renderFeats() {
  // 재주 탭 제거(v0.282) — 패널이 없으면 no-op. 여러 곳에서 호출되므로 안전 가드.
  if (!document.getElementById('feats-special')) return;
  // 유산 표시 — 아코디언
...
function cascadeRemoveFeats() {
  if (typeof _checkPrereqs !== 'function') return;
...
function renderEquipBrowseItems() {
  ...
  if (equipBrowseMode === 'armor') return renderArmorBrowseItems();
...
function _loadCompanions() {
  ...
  fetch('data/derived/companions.json?v=0.321').then(r => r.ok ? r.json() : null).then(j => {
...
function _loadConditions() {
  ...
  fetch('data/derived/conditions.json?v=0.321').then(r => r.ok ? r.json() : null).then(j => {
...
function _familiarAbilityIconUrl(id) { return FAMILIAR_ABILITY_ICON_BASE + (FAMILIAR_ABILITY_ICONS[id] || FAMILIAR_DEFAULT_ICON) + '?v=0.321'; }
function _familiarPatronIconUrl() { return FAMILIAR_ABILITY_ICON_BASE + FAMILIAR_PATRON_ICON + '?v=0.321'; }
...
function _familiarAbilityBoxHtml(icon, name, sub, desc, locked) {
  return `<div style="display:flex;align-items:flex-start;gap:8px;padding:6px 8px;background:var(--bg3);border:1px solid ${locked ? 'var(--accent)' : 'var(--border2)'};border-radius:6px;">
    <img src="${icon}" loading="lazy" style="width:28px;height:28px;border-radius:5px;flex-shrink:0;object-fit:cover;" onerror="this.src='${FAMILIAR_ABILITY_ICON_BASE + FAMILIAR_DEFAULT_ICON}?v=0.321'">
