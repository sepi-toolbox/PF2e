...
const PER_SHEET = {
  ...
  EFFECT_GROUPS: {
    group_id:        'EFFECT_GROUPS PK — FEAT_DB.effect_group_id 또는 CHOICE_OPTIONS.effect_group_id가 참조. eg-{feat.id} 또는 eg-{feat.id}-{option.id}',
    type:            '효과 타입 enum: hp_bonus, save_bonus, ac_bonus, speed_bonus, initiative_bonus, skill_bonus, skill_trained, grant_action, grant_focus_spell, grant_innate_spell, grant_feat, grant_feat_if_trained, grant_lore, grant_weapon, grant_adopted_feat, weapon_familiarity, weapon_trained, vision_upgrade, extra_sense, resistance, proficiency, cantrip_slots, spell_slots, dying_threshold, recovery_dc, familiar_abilities, speed_extra, bulk_bonus, unburdened_iron, martial_experience, adopted_ancestry, armor_upgrade ... (display_note/damage_note는 FEAT_DB.auto_note/damage_note로 흡수됨)',
    value:           '수치 값 (hp_bonus/save_bonus/speed_bonus/ac_bonus/bulk_bonus/cantrip_slots/dying_threshold/initiative_bonus/recovery_dc/speed_extra/spell_slots/familiar_abilities)',
    skill:           'SKILLS.id 외래키 (skill_trained / grant_feat_if_trained). $choice면 사용자 선택',
    save:            'will/fort/reflex/all (save_bonus)',
    ...
    spell:           '주문명 또는 동적 sentinel (grant_innate_spell/grant_focus_spell). $domain_initial/$domain_advanced 가능',
    tradition:       'arcane/divine/occult/primal (grant_innate_spell)',
    spell_type:      'spell/cantrip (grant_innate_spell)',
    uses:            '사용횟수 텍스트 ("하루 1회") (grant_innate_spell)',
    feat:            '부여 재주 (grant_feat/grant_feat_if_trained — "한글명 (영문명)" 형태)',
    default_choice:  '자식 재주의 기본 choice (grant_feat/grant_feat_if_trained)',
    weapons:         '무기 ID 배열 (weapon_familiarity/weapon_trained — 같은 종류 복수 OK)',
    weapon_name:     '부여 무기명 (grant_weapon)',
    ...
    vision:          '시야 enum ("암시야"/"상위 암시야") (vision_upgrade)',
    sense:           '감각 텍스트 (extra_sense)',
    damage_type:     '"정밀"/"냉기" 등 (resistance)',
    action:          'ACTION_DB 외래키 (grant_action — aoo 등)',
    summary:         'action ID 없는 인라인 행동 텍스트 (grant_action)',
    actionCost:      '1/2/3/free/reaction (grant_action)',
    name:            '지식 분야 한글명 (grant_lore)',
    target:          '무기/갑옷/기타 (proficiency — DOM id suffix)',
